...
@router.post("/{task_id}/request-changes", summary="Create a follow-up task with requested changes")
async def request_changes(
    task_id: str,
    body: RequestChangesBody,
    session: Annotated[AsyncSession, Depends(get_session)],
) -> dict:
    """Create a new task from change request on an existing task."""
    try:
        tid = uuid.UUID(task_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid task ID format")
    
    original = await session.get(Task, tid)
    if not original:
        raise HTTPException(status_code=404, detail="Task not found")
    ...
    new_task = Task(
        id=uuid.uuid4(),
        title=f"Правки: {original.title[:80]}",
        description=f"Правки к задаче \"{original.title}\":\n\n{body.comment}\n\n---\nОригинальная задача: {task_id}\nPR: {original.pr_url or 'N/A'}",
        status="queued",
        priority=body.priority,
        repo=original.repo,
        created_by="user",
...
@router.post("/{task_id}/release", summary="Merge PR and mark task as released")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                return {
                    "status": "released",
                    "message": f"PR #{task.pr_number} merged into develop",
                    "task_id": task_id,
                }
            else:
