...
async def deploy_branch_to_dev(repo: str, branch: str) -> dict:
    ...
    tmpdir = tempfile.mkdtemp(prefix="qa-deploy-")
        ...
        if not deploy_result.get("success"):
            async with session_factory() as session:
                await log_agent(
                    ...
                    "qa",
                    "error",
                    f"Deploy failed: {deploy_result.get('error', 'unknown')}",
                    task_id=task_id,
                )
                await session.commit()
        ...
        try:
            async with session_factory() as session:
                result = await session.execute(
                    select(Task).where(Task.status == TaskStatus.QA_TESTING).order_by(Task.created_at).limit(1)
                )
